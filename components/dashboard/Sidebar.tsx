      ...
      `}>
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsCollapsed(!isCollapsed)} className={`hidden lg:block p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all ${isCollapsed ? 'mx-auto' : ''}`}>
              {isCollapsed ? <PanelLeftOpen size={20}/> : <PanelLeftClose size={20}/>}
            </button>
            <button onClick={onClose} className="lg:hidden p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
              <X size={20}/>
            </button>
          </div>
        </div>
