...
export default function ActiveQuizPage() {
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-card bg-white/90 border border-slate-100 rounded-3xl p-8 text-center shadow-xl">
          ...
          <div className="flex justify-between pb-6 mb-6 border-b border-slate-100 px-4">
            ...
            <div className="text-center">
              ...
              <p className="font-bold text-amber-500">+{score * 10}</p>
            </div>
            <div className="text-center">
          ...
          <div className="space-y-3 pt-2">
            ...
            <Link href="/dashboard" className="w-full btn-gradient py-3.5 rounded-xl text-white font-bold flex items-center justify-center gap-2 shadow-md">
              <Home className="w-4 h-4" /> Dashboardga
            </Link>
            <Link href="/chat" className="w-full py-3.5 rounded-xl bg-transparent text-indigo-500 hover:text-indigo-700 font-bold flex items-center justify-center gap-2 transition-colors hover:bg-indigo-50/50">
              <MessageSquare className="w-4 h-4" /> AI bilan xatolarni ko'rib chiqish
            </Link>
          </div>
        </motion.div>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-20">
      {/* TOP NAVIGATION */}
      <div className="flex justify-between items-center bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-slate-100 shadow-sm sticky top-4 z-20">
        ...
        <div className="flex gap-4">
          ...
          <div className="text-sm font-bold text-amber-500 bg-amber-50 px-2.5 py-1 rounded-md border border-amber-100 flex items-center gap-1">
            +10 XP
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex gap-2 mb-6">
          <span className="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-md">Pifagor teoremasi</span>
          <span className="bg-amber-50 text-amber-700 text-xs font-bold px-3 py-1 rounded-md border border-amber-100">O'rta</span>
        </div>
        ...
        {/* GEOMETRY FIGURE */}
        <div className="bg-slate-50/80 rounded-2xl p-6 md:p-10 mb-8 border border-slate-100 flex items-center justify-center">
          <svg width="200" height="150" viewBox="0 0 200 150" className="overflow-visible">
            <path d="M40 130 L160 130 L40 40 Z" fill="none" stroke="#4F46E5" strokeWidth="3" strokeLinejoin="round" />
            {/* Right angle marker */}
            <path d="M40 115 L55 115 L55 130" fill="none" stroke="#4F46E5" strokeWidth="2" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {['5 sm', '6 sm', '7 sm', '4.5 sm'].map((opt, i) => {
            ...
            if (showWrongStatus) btnClass = "border-2 border-red-400 bg-red-50 text-red-800 shadow-sm";
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 border-2 ${
                  ...
                  isSelected ? 'bg-indigo-500 border-indigo-600 text-white' : 'bg-slate-100 border-slate-200 text-slate-500'
                }`}>
        ...
        <AnimatePresence>
          {isAnswered && selectedAnswer !== 0 && (
            <motion.div 
              ...
              className="bg-amber-50 border-l-4 border-amber-400 rounded-r-2xl p-5 mb-6 overflow-hidden"
            >
              <h4 className="font-bold text-amber-800 mb-1">To'g'ri javob: A) 5 sm</h4>
        ...
        <div className="flex justify-end pt-4 border-t border-slate-100">
          {!isAnswered ? (
            <button 
               ...
               className="btn-gradient px-8 py-3.5 rounded-xl font-bold text-white shadow-md disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-all"
            >
              Tekshirish
            </button>
          ) : (
            <motion.button 
               ...
               className="btn-gradient px-8 py-3.5 rounded-xl font-bold text-white shadow-md flex items-center gap-2 hover:-translate-y-0.5 transition-all"
            >
