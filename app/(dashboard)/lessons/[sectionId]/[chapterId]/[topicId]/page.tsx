...
export default function TopicPage() {
  ...
  async function completeLesson() {
    ...
    try {
      ...
      await setDoc(doc(db, 'userProgress', progressId), {
        ...
        completed: true,
        score: 100, // Read credit
        completedAt: serverTimestamp(),
      })
      await updateDoc(doc(db, 'users', user.uid), {
        totalLessonsCompleted: increment(1),
      })
      const earned = Math.round(topic.xpReward * 0.5) // Half reward for just reading
  ...
  return (
    ...
    >
      ...
      <div className="glass-card dark:bg-slate-800/60
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 items-center">
            {userData?.role === 'admin' && (
              <div className="flex bg-slate-100 dark:bg-slate-700/50 p-1 rounded-xl mr-2">
                <button
                  onClick={() => setActiveTab('content')}
                  className={`p-2 rounded-lg transition-all ${activeTab === 'content' ? 'bg-white dark:bg-slate-600 shadow-sm text-indigo-600' : 'text-slate-400'}`}
                >
                  <BookOpen size={16} />
                </button>
                <button
                  onClick={() => setActiveTab('editor')}
                  className={`p-2 rounded-lg transition-all ${activeTab === 'editor' ? 'bg-white dark:bg-slate-600 shadow-sm text-indigo-600' : 'text-slate-400'}`}
                >
                  <Settings size={16} />
                </button>
              </div>
      ...
      {activeTab === 'editor' && userData?.role === 'admin' ? (
        <QuizEditor 
          topicId={topicId as string}
          sectionId={sectionId as string}
          chapterId={chapterId as string}
              ...
              {!completed ? (
                <button
                  ...
                  className="btn-gradient flex-1 py-3 rounded-xl
                             ...
                             justify-center gap-2 disabled:opacity-75"
                >
                  <CheckCircle2 size={16} />
                  {completing ? 'Yakunlanmoqda...' : 'Darsni yakunlash'}
                </button>
              ) : (
                <Link href={`/lessons/${sectionId}`}
